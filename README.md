...
# Video Orientation Sorter Toolkit

Automatically sort video files into **horizontal** and **vertical** folders based on their resolution metadata. Two flavors available:

- **Windows** → PowerShell (3-step setup)
- **Linux / macOS** → Bash (1-step setup)
...
- **unreadable metadata** → **skipped**, left in place
...
### Quick Start (Script File)

Copy `video_sorter.ps1` into your video folder and run it:
...
- **Bash 4+** (pre-installed on virtually all Linux/macOS systems)
- **ffprobe** — install via your package manager (one command)

### Step 1 — Install FFmpeg / FFprobe

A single package manager command is all you need:
...
That's it. No extra setup, no PATH editing — your package manager handles everything.

### Step 2 — Sort Videos by Orientation

Navigate to your video folder and run the script:

```bash
cd /path/to/your/videos

# Option A: Run the script file
chmod +x sort_videos.sh
./sort_videos.sh

# Option B: Paste directly into your terminal
bash sort_videos.sh
```

**The script (`sort_videos.sh`):**
...
| Line | Action |
|------|--------|
| `mkdir -p horizontal vertical` | Creates output folders (silently skips if they exist) |
| `for f in *.*; do` | Iterates over every file in the current directory |
| `if [ "$f" == "sort_videos.sh" ]; then continue; fi` | Skips the script itself so it doesn't get moved |
| `res=$(ffprobe ...)` | Queries the video's width×height via ffprobe |
| `width=... height=...` | Splits the resolution string on `x` |
| `if [[ "$width" =~ ^[0-9]+$ ]] ...` | Validates that both values are numeric |
| `mv "$f" horizontal/` or `vertical/` | Moves the file to the matching folder |
| Square files (width = height) | Silently skipped — left in place |
...
FFmpeg/FFprobe is not installed or not in your PATH.

**Windows:**
1. Restart your terminal — the PATH change only takes effect in new sessions.
2. Verify `$HOME/FFmpeg/` contains a `bin` folder with `ffprobe.exe`.
3. Add the `bin` folder to your User PATH manually via **System Properties → Environment Variables**.

**Linux/macOS:**
1. Run `which ffprobe` to check if it's installed.
2. If missing, install via your package manager (see [Step 1](#step-1--install-ffmpeg--ffprobe-1)).

### `Unable to parse video streams` / files not getting sorted

This can happen when:
- The file is **corrupted** or not a valid video.
- The video uses a codec your ffprobe build doesn't support (rare).
- The file is **locked** by another process.
- (Bash) The file has **no extension** — the script iterates `*.*`, so extensionless files are still checked; if ffprobe can't read them, they'll be skipped.

### NuGet provider not found (Windows only)

- Run PowerShell as your normal user (not admin unless required).
- Run `Get-PackageProvider -ListAvailable` and look for `NuGet`.
...
**Bash:**
```bash
mv horizontal/* vertical/* . 2>/dev/null
```
